...
"""
    Soft Actor Critic
    Modification of: https://github.com/XinJingHao/SAC-Continuous-Pytorch
"""


class RandomBuffer(object):
    def __init__(self, state_dim, action_dim, max_size=int(1e4), device='cuda'):
    ...
    def sample(self, batch_size):
        ...
        ind = torch.randint(0, self.size, size=(1, batch_size)).squeeze()
        with torch.no_grad():
            return (
    ...
    # error, need to rewrite for tensor
    def save(self):
...
# Build net with for loop: multi-layer MLP with activation function
...
class Actor(nn.Module):
    ...
    def forward(self, state, deterministic=False, with_logprob=True):
        ...
        # a = torch.tanh(u)   # dai norm_action
        a = self.action_dim * F.softmax(u, dim=-1)
...
class SAC_Agent(nn.Module):
    ...
    def train(self, replay_buffer, k):
        s, a, r, s_prime = replay_buffer.sample(int(self.batch_size * (k / 2)))
        # s, a, r, s_prime = replay_buffer.sample(self.batch_size)
        ...
        with torch.no_grad():
            ...
            target_Q = r + self.gamma * (target_Q - self.alpha * log_pi_a_prime)
    ...
    def save(self, episode):
        torch.save(self.actor.state_dict(), "./rl_model/sac_actor{}.pth".format(episode))
        torch.save(self.q_critic.state_dict(), "./rl_model/sac_q_critic{}.pth".format(episode))

    def load(self, episode):
        self.actor.load_state_dict(torch.load("./rl_model/sac_actor{}.pth".format(episode)))
        self.q_critic.load_state_dict(torch.load("./rl_model/sac_q_critic{}.pth".format(episode)))