...
class NashMTL(WeightMethod):
    ...
    def _stop_criteria(self, gtg, alpha_t):
        return ((self.alpha_param.value is None)
...
class PCGrad(WeightMethod):
    ...
    def _project_conflicting(self, grads: List[Tuple[torch.Tensor]]):
        ...
        for g_i in pc_grad:
            ...
            for g_j in grads:
                g_i_g_j = sum([torch.dot(torch.flatten(grad_i), torch.flatten(grad_j))
...
class CAGrad(WeightMethod):
    ...
    def cagrad(self, grads, alpha=0.5, rescale=1):
        ...
        def objfn(x):
            return (x.reshape(1, self.n_tasks).dot(A).dot(b.reshape(self.n_tasks, 1))
                    + c * np.sqrt(x.reshape(1, self.n_tasks).dot(A).dot(x.reshape(self.n_tasks, 1)) + 1e-8)).sum()
...
class GradientWeightMethods:
    def __init__(self, method: str, n_tasks: int, device: torch.device, **kwargs):
        ...
        :param method:
        """
        assert method in list(GRADIENT_METHODS.keys()), f"unknown method {method}."

        self.method = GRADIENT_METHODS[method](n_tasks=n_tasks, device=device, **kwargs)
